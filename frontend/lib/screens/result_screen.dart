...
class ResultScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // ── Parse flat + nested fields from mock/real API ─────────
    ...
    final confidence = (data['confidence_level'] ?? (glm['confidence'] != null
        ? (glm['confidence'] as num) / 100 : 0.0)) as num;
    ...
    final lhdnReference     = data['lhdn_reference']     ??
        (glm['citations'] as List?)?.join(', ')          ?? 'N/A';
    final recommendation    = data['action_recommendation'] ??
        (glm['tax_saving_tips'] as List?)?.first         ?? 'N/A';
    ...
    final disclaimer = glm['disclaimer']   ?? data['disclaimer']
        ?? 'For pre-audit analysis only. Consult a licensed tax advisor.';
    ...
    if (status == 'DANGER' || status == 'INVALID') {
      cardColor = Colors.red; statusIcon = Icons.dangerous_rounded;
    }

    return Scaffold(
      backgroundColor: const Color(0xFFF6FAFE),
      appBar: AppBar(
        title: Text('Compliance Verdict',
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(
            ...
            child: Column(children: [
              ...
              // Metrics row
              Row(children: [
                _metricTile('Fine Exposure',
                    'RM ${impactSaved.toStringAsFixed(0)}',
                    Colors.red.shade50, Colors.red),
                const SizedBox(width: 8),
                _metricTile('Risk Score',
                    '${riskScore.toInt()}/100',
                    Colors.blue.shade50, Colors.blue),
                const SizedBox(width: 8),
                _metricTile('Tax Amount',
          ...
          _sectionCard(
            ...
            iconColor: kNavy,
            title: 'Extracted Receipt Data',
            child: Column(children: [
              _dataRow('Merchant', merchantName),
              _divider(),
              _dataRow('Tax ID (TIN)', tin,
                  valueColor: tin == 'NOT_FOUND' ? Colors.red : Colors.black87),
              _divider(),
              _dataRow('Date', date),
              _divider(),
              _dataRow('Total Amount',
          ...
          _sectionCard(
            ...
            iconColor: Colors.indigo,
            title: 'LHDN Citations',
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: citations.isNotEmpty
                  ? citations
                      .map((c) => _bulletPoint(c.toString(), Colors.indigo))
                      .toList()
                  : [Text(lhdnReference,
          ...
          _sectionCard(
            ...
            child: Column(
              ...
              children: tips.isNotEmpty
                  ? tips
                      .map((t) => _bulletPoint(
                          t.toString(), Colors.amber.shade700))
                      .toList()
                  : [_bulletPoint(recommendation.toString(),
          ...
          Container(
            ...
            child: Row(children: [
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Potential Capital Protected',
                    style: TextStyle(
                        fontSize: 12,
                        color: Colors.grey,
                        fontWeight: FontWeight.w500)),
                Text('RM ${impactSaved.toStringAsFixed(2)}',
                    style: TextStyle(
                        fontSize: 22,
                        fontWeight: FontWeight.bold,
  ...
  // ── Helper widgets ────────────────────────────────────────────
  Widget _metricTile(
      String label, String value, Color bg, MaterialColor color) {
    return Expanded(
      child: Container(
        padding:
  ...
  Widget _dataRow(String label, String value,
      {bool bold = false, Color valueColor = Colors.black87}) {
    return Padding(
